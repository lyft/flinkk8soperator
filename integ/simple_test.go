...
func updateAndValidate(c *C, s *IntegSuite, name string, updateFn func(app *v1beta2.FlinkApplication), failurePhase v1beta2.FlinkApplicationPhase) *v1beta2.FlinkApplication {
	...
	c.Assert(err, IsNil)
	c.Assert(newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID, Not(Equals), app.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID)

	log.Info("New job started successfully")

	// check that we savepointed and restored correctly
	endpoint := fmt.Sprintf("jobs/%s/checkpoints", newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID)
...
func (s *IntegSuite) TestSimple(c *C) {
...

	{
		...
		jobID := newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID
		newApp, err = s.Util.GetFlinkApplication(newApp.Name)
		c.Assert(err, IsNil)
		c.Assert(newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID, Not(Equals), jobID)
...

	{
		...
		c.Assert(newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.State, Equals, v1beta2.Running)
	...
	job := func() map[string]interface{} {
		...
		for _, j := range jobList {
			...
			if job["id"] == app.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID {
				return job
			}
		}
		return nil
...
func (s *IntegSuite) TestRecovery(c *C) {
	...
	endpoint := fmt.Sprintf("jobs/%s/checkpoints", app.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID)
	for {
	...
	for {
		...
		newApp, err := s.Util.GetFlinkApplication(config.Name)
		c.Assert(err, IsNil)
		if newApp.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID != app.Status.ApplicationStatus[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID {
			break
		}
		time.Sleep(100 * time.Millisecond)
	}
