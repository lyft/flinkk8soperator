...
func updateAndValidate(c *C, s *IntegSuite, name string, updateFn func(app *v1beta1.FlinkApplication), failurePhase v1beta1.FlinkApplicationPhase) *v1beta1.FlinkApplication {
	...
	c.Assert(err, IsNil)
	c.Assert(newApp.Status.ApplicationStatus[0].JobStatus.JobID, Not(Equals), app.Status.ApplicationStatus[0].JobStatus.JobID)

	log.Info("New job started successfully")

	// check that we savepointed and restored correctly
	endpoint := fmt.Sprintf("jobs/%s/checkpoints", newApp.Status.ApplicationStatus[0].JobStatus.JobID)
...
func (s *IntegSuite) TestSimple(c *C) {
...

	{
		...
		jobID := newApp.Status.ApplicationStatus[0].JobStatus.JobID
		newApp, err = s.Util.GetFlinkApplication(newApp.Name)
		c.Assert(err, IsNil)
		c.Assert(newApp.Status.ApplicationStatus[0].JobStatus.JobID, Not(Equals), jobID)
...

	{
		...
		c.Assert(newApp.Status.ApplicationStatus[0].JobStatus.State, Equals, v1beta1.Running)
	...
	job := func() map[string]interface{} {
		...
		for _, j := range jobList {
			...
			if job["id"] == app.Status.ApplicationStatus[0].JobStatus.JobID {
				return job
			}
		}
		return nil
...
func (s *IntegSuite) TestRecovery(c *C) {
	...
	endpoint := fmt.Sprintf("jobs/%s/checkpoints", app.Status.ApplicationStatus[0].JobStatus.JobID)
	for {
	...
	for {
		...
		newApp, err := s.Util.GetFlinkApplication(config.Name)
		c.Assert(err, IsNil)
		if newApp.Status.ApplicationStatus[0].JobStatus.JobID != app.Status.ApplicationStatus[0].JobStatus.JobID {
			break
		}
		time.Sleep(100 * time.Millisecond)
	}
