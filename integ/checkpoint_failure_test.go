...
func failingJobTest(s *IntegSuite, c *C, testName string, causeFailure func()) {
	...
	c.Assert(err, IsNil)
	c.Assert(newApp.Status.VersionStatuses[s.Util.GetCurrentStatusIndex(newApp)].JobStatus.JobID, Equals, app.Status.VersionStatuses[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID)

	endpoint := fmt.Sprintf("jobs/%s", app.Status.VersionStatuses[s.Util.GetCurrentStatusIndex(app)].JobStatus.JobID)
